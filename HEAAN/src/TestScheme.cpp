...
void TestScheme::test() {
	...
	long slots = (1 << logSlots);
	CZZ* mvec1 = EvaluatorUtils::evalRandCZZArray(slots, logp);
	CZZ* mvec2 = EvaluatorUtils::evalRandCZZArray(slots, logp);
	//-----------------------------------------
	timeutils.start("encode");
	Plaintext msg1 = scheme.encode(mvec1, slots, logQ);
	timeutils.stop("encode");

	timeutils.start("encrypt msg");
	Ciphertext cipher1 = scheme.encryptMsg(msg1);
	timeutils.stop("encrypt msg");

	timeutils.start("Encrypt batch");
	Ciphertext cipher2 = scheme.encrypt(mvec2, slots, logQ);
	timeutils.stop("Encrypt batch");

	//-----------------------------------------
	timeutils.start("mult batch");
	scheme.multAndEqual(cipher1, cipher2);
	timeutils.stop("mult batch");

	timeutils.start("mult");
	Ring2Utils::multAndEqual(cipher1.ax, cipher2.bx, context.Q, context.N);
	timeutils.stop("mult");
